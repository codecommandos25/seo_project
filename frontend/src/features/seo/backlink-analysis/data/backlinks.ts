...
export const backlinks = Array.from({ length: 20 }, () => {
  ...
  return {
    ...
    sourcePageTitle: faker.lorem.sentence(faker.number.int({ min: 3, max: 10 })),
    ...
    domainAuthority: faker.number.float({ min: 0, max: 100, fractionDigits: 1 }),
    pageAuthority: faker.number.float({ min: 0, max: 100, fractionDigits: 1 }),
    firstSeen,
    lastSeen: faker.date.between({ from: firstSeen, to: new Date() }),
    statusCode: faker.helpers.maybe(() => faker.helpers.arrayElement([200, 301, 302, 404, 500])),
    isLive: faker.helpers.maybe(() => faker.datatype.boolean(), { probability: 0.8 }),
  }
})
