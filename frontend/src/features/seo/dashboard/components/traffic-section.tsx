...
export function TrafficSection() {
  ...
  React.useEffect(() => {
    if (graphData) {
      // Combine the data
      const combinedData =
        graphData?.tasks[0].result[0].items[0].metrics.organic.map(
          (organicItem: any) => {
            // Find matching paid item
            const paidItem =
              graphData?.tasks[0].result[0].items[0].metrics.paid.find(
                (item: any) =>
                  item.year === organicItem.year &&
                  item.month === organicItem.month
              )
  ...
  console.log(
    'graph organic',
    graphData?.tasks[0].result[0].items[0].metrics.organic
  )
  ...
  return (
    <Card>
      ...
      <div className='flex justify-end px-6 py-4'>
        <Select
          value={timeRange}
          onValueChange={(value) => setTimeRange(value as '1y' | '6m')}
        >
    ...
    /> */}
              ...
              <Bar
                dataKey={`${activeChart}`}
                fill={`${activeChart == 'organic' ? chartConfig.organic.color : chartConfig.paid.color}`}
              />
            </BarChart>
          </ChartContainer>
        </CardContent>
