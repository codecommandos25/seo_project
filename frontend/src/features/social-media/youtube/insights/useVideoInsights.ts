...
const generateMockVideoInsights = (video: YouTubeVideo): VideoInsight => {
    ...
    const calculateAverageDuration = (retentionPoints: RetentionPoint[]): string => {
        const totalDuration = isShort ?
            parseInt(video.duration.split(':')[1]) :
            parseInt(video.duration.split(':')[0]) * 60 + parseInt(video.duration.split(':')[1]);
    ...
    const totalRetention = retentionPoints.reduce((sum, point) => sum + point.retentionPercentage, 0);
    ...
    const generateAudienceSources = (): AudienceSource[] => {
        const sources = [
            { source: 'Subscribers', percentage: faker.number.int({ min: 15, max: 45 }) },
    ...
    const generateTrafficSources = (): TrafficSource[] => {
        const sources = [
            {
                ...
                percentage: faker.number.int({ min: isShort ? 5 : 15, max: isShort ? 15 : 40 }),
                views: 0
            },
            {
        ...
        const allocatedPercentage = sources.slice(0, 3).reduce((sum, src) => sum + src.percentage, 0);
    ...
    const impressions = video.views * faker.number.float({ min: 5, max: 20, precision: 0.1 });
    ...
    return {
        ...
        viewsPerHour: daysAgo <= 2 ? Math.floor(video.views / (daysAgo * 24 || 1)) : 0,
        viewsLast48Hours: daysAgo <= 30 ? Math.floor(video.views * (daysAgo <= 2 ? 1 : 0.15)) : 0,
        viewsLast7Days: daysAgo <= 30 ? Math.floor(video.views * (daysAgo <= 7 ? 1 : 0.4)) : 0,
        viewsLast30Days: daysAgo <= 60 ? Math.floor(video.views * (daysAgo <= 30 ? 1 : 0.7)) : 0,
        subscribersGained: Math.floor(video.views * faker.number.float({ min: 0.001, max: 0.05, precision: 0.001 })),
        impressions: Math.floor(impressions),
        audienceRetention: retentionPoints,
        audienceSourcesData: generateAudienceSources(),
        trafficSources,
        performanceScore: calculatePerformanceScore(),
        recommendationImpressions: Math.floor(impressions *
            (trafficSources.find(s => s.source === 'Suggested videos')?.percentage || 20) / 100),
    };
};
...
const fetchVideoInsights = async (videoId: string): Promise<VideoInsight> => {
    ...
    const mockVideo: YouTubeVideo = {
        ...
        thumbnailUrl: faker.image.urlLoremFlickr({ category: 'technology', width: 1280, height: 720 }),
