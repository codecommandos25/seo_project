...
const generateTopPerformingVideos = (count: number): TopPerformingVideo[] => {
  return Array.from({ length: count }, () => {
    const type = faker.helpers.arrayElement(['short', 'long']) as
      | 'short'
      | 'long'
...
const generateDemographicData = (): DemographicData[] => {
  ...
  for (let i = 0; i < ageRanges.length - 1; i++) {
    ...
    percentage = Math.min(
      percentage,
      remainingPercentage - (ageRanges.length - i - 1)
    )
    ...
    demographics.push({
      ...
      malePercentage,
      femalePercentage:
        100 - malePercentage - faker.number.int({ min: 0, max: 2 }),
...
const generateCountryData = (): CountryData[] => {
  ...
  for (let i = 0; i < countries.length - 1; i++) {
    ...
    percentage = Math.min(
      percentage,
      remainingPercentage - (countries.length - i - 1)
    )
    remainingPercentage -= percentage
  ...
  const actionTemplates: RecommendedAction[] = [
    {
      ...
      description:
        'Shorts are driving significant engagement. Try to publish 2-3 shorts weekly.',
      impact: 'high',
      category: 'content',
    },
    {
      ...
      description:
        'Your top-performing videos could reach more viewers with better keyword optimization.',
      impact: 'medium',
      category: 'seo',
    },
    {
      ...
      description:
        'Several videos have below-average click-through rates. Revamped thumbnails could improve performance.',
      impact: 'high',
      category: 'seo',
    },
    {
      ...
      description:
        'Posting more community posts can boost subscriber retention and video views.',
      impact: 'medium',
      category: 'engagement',
    },
    {
      ...
      description:
        'Your 7-12 minute videos have the highest retention. Consider this length for future content.',
      impact: 'medium',
      category: 'content',
    },
    {
    ...
    },
    {
      ...
      description:
        'Your channel may benefit from diversifying revenue streams beyond AdSense.',
      impact: 'high',
    ...
    },
    {
      ...
      description:
        'Create content related to trending topics in your niche to capture search traffic.',
      impact: 'high',
      category: 'content',
    },
  ]
...
const fetchChannelInsights = async (): Promise<ChannelInsights> => {
  ...
  const avgDurationMinutes = faker.number.float({
    min: 3,
    max: 6,
    precision: 0.1,
  })
  ...
  return {
    ...
    totalVideos,
    watchTimeHours,
    averageViewDuration: `${Math.floor(avgDurationMinutes)}:${Math.floor(
      (avgDurationMinutes % 1) * 60
    )
      .toString()
      .padStart(2, '0')}`,
