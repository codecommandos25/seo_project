...
const generateMockRevenueAnalytics = (): RevenueAnalytics => {
  ...
  const generateMonthlyRevenue = (): MonthlyRevenue[] => {
    ...
    const sponsorshipGrowth =
      1 + faker.number.float({ min: -0.03, max: 0.15 }) / 12
    const membershipGrowth =
      1 + faker.number.float({ min: 0.01, max: 0.08 }) / 12
    const merchandiseGrowth =
      1 + faker.number.float({ min: -0.08, max: 0.2 }) / 12
    const superChatsGrowth =
      1 + faker.number.float({ min: -0.02, max: 0.1 }) / 12
    ...
    for (let i = 0; i < months.length; i++) {
      ...
      // Add some randomness
      const adRevenue = Math.round(
        baseAdRevenue * faker.number.float({ min: 0.9, max: 1.1 })
      )
      const sponsorshipRevenue = Math.round(
        baseSponsorshipRevenue * faker.number.float({ min: 0.85, max: 1.15 })
      )
      const membershipRevenue = Math.round(
        baseMembershipRevenue * faker.number.float({ min: 0.95, max: 1.05 })
      )
      const merchandiseRevenue = Math.round(
        baseMerchandiseRevenue * faker.number.float({ min: 0.8, max: 1.2 })
      )
      const superChatsRevenue = Math.round(
        baseSuperChatsRevenue * faker.number.float({ min: 0.85, max: 1.15 })
      )
  ...
  const generateRevenueBySource = (): RevenueSource[] => {
    const totalAdRevenue = monthlyRevenue.reduce(
      (sum, month) => sum + month.adRevenue,
      0
    )
    const totalSponsorshipRevenue = monthlyRevenue.reduce(
      (sum, month) => sum + month.sponsorshipRevenue,
      0
    )
    const totalMembershipRevenue = monthlyRevenue.reduce(
      (sum, month) => sum + month.membershipRevenue,
      0
    )
    const totalMerchandiseRevenue = monthlyRevenue.reduce(
      (sum, month) => sum + month.merchandiseRevenue,
      0
    )
    const totalSuperChatsRevenue = monthlyRevenue.reduce(
      (sum, month) => sum + month.superChatsRevenue,
      0
    )
    ...
    const sources: RevenueSource[] = [
      ...
      {
        source: 'Sponsorships',
        amount: totalSponsorshipRevenue,
        percentage: 0,
      },
  ...
  const calculateRevenueProjection = (): RevenueProjection => {
    ...
    return {
      ...
      nextQuarter: Math.round(
        avgRecentRevenue * 3 * (1 + (growthRate / 100) * 2)
      ),
  ...
  const generateAdPerformance = (): AdPerformance => {
    ...
    adTypes.forEach((adType) => {
      adType.cpm = parseFloat(
        ((adType.revenue / adType.impressions) * 1000).toFixed(2)
      )
    })
    ...
    // Calculate totals
    const totalImpressions = adTypes.reduce(
      (sum, adType) => sum + adType.impressions,
      0
    )
    const totalRevenue = adTypes.reduce(
      (sum, adType) => sum + adType.revenue,
      0
    )
    const averageCpm = parseFloat(
      ((totalRevenue / totalImpressions) * 1000).toFixed(2)
    )

    // Generate playback-based CPM
    const playbackLocations = [
      'Watch page',
      'Channel page',
      'Embedded',
      'Shorts feed',
  ...
  const generateSponsorshipPerformance = (): SponsorshipPerformance[] => {
    ...
    return sponsorshipNames
      .map((name) => {
        const status = faker.helpers.arrayElement([
          'active',
          'completed',
          'upcoming',
        ]) as 'active' | 'completed' | 'upcoming'
  ...
  const generateMembershipStats = (): MembershipStats => {
    ...
    const totalMembershipRevenue = tierDistribution.reduce(
      (sum, tier) => sum + tier.revenue,
      0
    )
    const averageRevenuePer = parseFloat(
      (totalMembershipRevenue / totalMembers).toFixed(2)
    )
  ...
  const generateCPMTrends = (): CPMTrend[] => {
    ...
    for (let i = 0; i < months.length; i++) {
      ...
      if (i >= 9 && i <= 11) {
        // Q4 has higher CPM
        seasonalFactor = faker.number.float({
          min: 1.2,
          max: 1.6,
          precision: 0.01,
        })
      ...
      // Add some randomness
      const randomFactor = faker.number.float({
        min: 0.9,
        max: 1.1,
        precision: 0.01,
      })
      ...
      const cpm = parseFloat(
        (baseCPM * seasonalFactor * randomFactor).toFixed(2)
      )
      trends.push({
        date: months[i],
        cpm,
  ...
  const revenueRetention = {
    ...
    viewsToRevenueRatio: faker.number.float({
      min: 1.5,
      max: 6,
      precision: 0.1,
    }),
  ...
  const previousPeriodRevenue =
    totalRevenue /
    (1 + faker.number.float({ min: -0.1, max: 0.4, precision: 0.01 }))
  ...
  return {
    summary: {
      ...
      revenuePerThousandViews: faker.number.float({
        min: 2,
        max: 8,
        precision: 0.1,
      }),
