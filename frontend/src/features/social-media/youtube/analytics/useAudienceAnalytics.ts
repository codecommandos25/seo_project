...
const generateMockAudienceAnalytics = (): AudienceAnalytics => {
    ...
    const totalAge = ageDemographics.reduce((sum, item) => sum + item.percentage, 0);
    ...
    const newTotal = ageDemographics.reduce((sum, item) => sum + item.percentage, 0);
    if (newTotal !== 100) {
        const diff = 100 - newTotal;
        const largest = ageDemographics.sort((a, b) => b.percentage - a.percentage)[0];
        largest.percentage += diff;
    }

    // Generate gender demographics
    const malePercentage = faker.number.int({ min: 55, max: 75 });
    const femalePercentage = faker.number.int({ min: 23, max: 100 - malePercentage - 1 });
    ...
    const deviceTotal = mobilePercentage + desktopPercentage + tabletPercentage + tvPercentage;

    const deviceDemographics: DeviceDemographics[] = [
        { device: 'Mobile', percentage: Math.round((mobilePercentage / deviceTotal) * 100) },
        { device: 'Desktop', percentage: Math.round((desktopPercentage / deviceTotal) * 100) },
        { device: 'Tablet', percentage: Math.round((tabletPercentage / deviceTotal) * 100) },
        { device: 'TV', percentage: Math.round((tvPercentage / deviceTotal) * 100) }
    ];

    // Ensure device percentages sum to 100
    const totalDevices = deviceDemographics.reduce((sum, item) => sum + item.percentage, 0);
    if (totalDevices !== 100) {
        deviceDemographics[0].percentage += (100 - totalDevices);
    }

    // Generate country data
    const countries = ['United States', 'India', 'United Kingdom', 'Canada', 'Australia', 'Germany', 'Brazil', 'Nigeria', 'Philippines', 'Other'];
    ...
    const totalCountryPercentage = countryData.reduce((sum, item) => sum + item.percentage, 0);
    countryData.forEach(item => {
        item.percentage = Math.round((item.percentage / totalCountryPercentage) * 100);
        item.subscriberCount = Math.round((item.percentage / 100) * totalSubscribers);
    });
    ...
    const subscriberActivity = days.map(day => ({
        day,
        activePercentage: faker.number.int({ min: 20, max: day === 'Saturday' || day === 'Sunday' ? 65 : 50 })
    }));

    // Generate trend data (30 days)
    const generateTrendData = (min: number, max: number, trend: 'up' | 'down' | 'stable' = 'up'): TrendData[] => {
        ...
        for (let i = 29; i >= 0; i--) {
            ...
            if (trend === 'up') {
                changeFactor = faker.number.float({ min: -0.03, max: 0.08, precision: 0.01 });
            } else if (trend === 'down') {
                changeFactor = faker.number.float({ min: -0.08, max: 0.03, precision: 0.01 });
            } else { // stable
                changeFactor = faker.number.float({ min: -0.03, max: 0.03, precision: 0.01 });
            }
    ...
    const subscribersDates: DateDistribution[] = Array.from({ length: 24 }, (_, hour) => {
        let percentage: number;
        if (hour >= 17 && hour <= 23) { // Evening hours
            percentage = faker.number.float({ min: 6, max: 10, precision: 0.1 });
        } else if (hour >= 9 && hour <= 16) { // Day hours
    ...
    // Adjust to 100%
    const totalHourPercentage = subscribersDates.reduce((sum, item) => sum + item.percentage, 0);
    subscribersDates.forEach(item => {
        item.percentage = parseFloat((item.percentage / totalHourPercentage * 100).toFixed(1));
    });
    ...
    const generateTopSubscribedVideos = (): TopSubscribedVideo[] => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const subscribersLost = faker.number.int({ min: 10, max: subscribersGained * 0.4 });
            const netGain = subscribersGained - subscribersLost;
            const views = faker.number.int({ min: subscribersGained * 10, max: subscribersGained * 30 });
            const conversionRate = parseFloat((subscribersGained / views * 100).toFixed(2));

            videos.push({
                id: faker.string.alphanumeric(11),
                title: faker.lorem.sentence(),
                thumbnailUrl: faker.image.urlLoremFlickr({ width: 1280, height: 720, category: 'technology' }),
                subscribersGained,
                subscribersLost,
                netGain,
                conversionRate,
    ...
    const audienceTypes: AudienceType[] = [
        { type: 'Casual Viewers', percentage: faker.number.int({ min: 35, max: 50 }) },
        { type: 'Dedicated Fans', percentage: faker.number.int({ min: 20, max: 35 }) },
        { type: 'Creators', percentage: faker.number.int({ min: 10, max: 25 }) },
        { type: 'Industry Professionals', percentage: faker.number.int({ min: 5, max: 15 }) }
    ];

    // Adjust to 100%
    const totalAudiencePercentage = audienceTypes.reduce((sum, item) => sum + item.percentage, 0);
    audienceTypes.forEach(item => {
        item.percentage = Math.round((item.percentage / totalAudiencePercentage) * 100);
    });

    // Generate traffic source data
    const trafficSources = ['Suggested videos', 'Browse features', 'YouTube search', 'External', 'Direct or unknown'];
    ...
    return {
        ...
        audienceTypes,
        returningViewerPercentage: faker.number.float({ min: 20, max: 45, precision: 0.1 }),
