...
const generateMockAnalytics = (): FacebookAnalyticsData => {
    ...
    const totalDemographic = demographics.reduce((total, item) => total + item.percentage, 0);
    demographics.forEach(item => {
        item.percentage = Math.round((item.percentage / totalDemographic) * 100);
    });

    // Ensure it adds up to 100%
    let remaining = 100 - demographics.reduce((total, item) => total + item.percentage, 0);
    demographics[0].percentage += remaining;

    // Mock location data
    const locations: LocationData[] = [
        { country: 'United States', percentage: faker.number.int({ min: 30, max: 60 }) },
        { country: 'United Kingdom', percentage: faker.number.int({ min: 10, max: 20 }) },
    ...
    const totalLocation = locations.reduce((total, item) => total + item.percentage, 0);
    locations.forEach(item => {
        item.percentage = Math.round((item.percentage / totalLocation) * 100);
    });

    // Ensure it adds up to 100%
    remaining = 100 - locations.reduce((total, item) => total + item.percentage, 0);
