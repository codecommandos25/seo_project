...
const generateMockPosts = (count: number): LinkedInPost[] => {
    const postTypes: ('text' | 'image' | 'article')[] = ['text', 'image', 'article']
    ...
    return Array.from({ length: count }, () => {
        ...
        return {
            ...
            mediaType,
            mediaUrl: hasImage ? faker.image.urlLoremFlickr({ category: 'business', width: 1200, height: 630 }) : undefined,
            articleTitle: isArticle ? faker.company.catchPhrase() : undefined,
            articleSummary: isArticle ? faker.lorem.sentences({ min: 1, max: 2 }) : undefined,
