...
const generateMockPersonalAnalytics = (): LinkedInPersonalAnalytics => {
  ...
  const totalProfileViews = dailyProfileViews.reduce(
  ...
  const totalPercentage = viewsByCompany.reduce(
    (sum, company) => sum + company.percentage,
    0
  )
  viewsByCompany.forEach((company) => {
    company.percentage = Math.round(
      (company.percentage / totalPercentage) * 100
    )
  })

  // Adjust to ensure total is 100%
  const diff =
    100 - viewsByCompany.reduce((sum, company) => sum + company.percentage, 0)
...
const generateMockCompanyAnalytics = (): LinkedInCompanyAnalytics => {
  ...
  const followerGrowthHistory = Array.from({ length: 6 }).map((_, index) => {
    ...
    const growthRate =
      1 + faker.number.float({ min: 0.01, max: 0.08 }) * (index + 1)
  ...
  // Normalize percentages to total 100%
  const totalPercentage = followerDemographics.reduce(
    (sum, demo) => sum + demo.percentage,
    0
  )
  ...
  // Adjust to ensure total is 100%
  const diff =
    100 - followerDemographics.reduce((sum, demo) => sum + demo.percentage, 0)
  followerDemographics[0].percentage += diff

  // Calculate current followers (last month's value)
  const currentFollowers =
    followerGrowthHistory[followerGrowthHistory.length - 1].followers

  // Calculate growth percentage based on last two months
  const previousFollowers =
    followerGrowthHistory[followerGrowthHistory.length - 2]?.followers ||
    currentFollowers
