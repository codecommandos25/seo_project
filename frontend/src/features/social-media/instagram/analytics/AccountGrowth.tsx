...
export default function AccountGrowth() {
    ...
    const metrics = {
        ...
        growthPercentage: data && data.length > 1
            ? (((data[data.length - 1]?.followers - data[0]?.followers) / data[0]?.followers) * 100).toFixed(1)
            : '0',
        totalReach: data
            ? data.reduce((sum, item) => sum + item.reach, 0)
            : 0,
        avgImpressions: data && data.length
            ? Math.round(data.reduce((sum, item) => sum + item.impressions, 0) / data.length)
            : 0
    }
        ...
        <Card>
            <CardHeader>
                ...
                <Tabs
                    ...
                    onValueChange={(value) => setTimeRange(parseInt(value) as 30 | 60 | 90)}
                    className="mt-2"
                >
                    <TabsList>
            ...
            <CardContent>
                ...
                <div className="mt-6">
                    <h3 className="text-sm font-medium mb-3">Follower Growth Trend</h3>
                    {renderContent()}
                </div>
            </CardContent>
        </Card>
