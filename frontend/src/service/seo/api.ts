...
type FilterNew = [string, string, boolean]

interface TargetObject {
  target: string
  mode: 'as_is' | string // 'as_is' or extend with other string literals
  filters: FilterNew
  limit: number
}
...
export const SEOKeywordsTable = (data: SearchQuery[]) =>
  POST({
    // url: `${ROUTES.AUTH_URL}/users/update-password`,
    // url: 'https://sandbox.dataforseo.com/v3/domain_analytics/whois/overview/live',
    url: 'http://13.234.181.212:8000/third-party-apis/ranked_keywords/table',
    data,
  })

export const pageOnSEO = (data: {
  target: string
  limit: number
  offset: number
}) =>
  POST({
    url: 'http://13.234.181.212:8000/third-party-apis/on-page-seo',
    data,
  })

export const CrawledSEOTable = (data: { target: string; limit: number }) =>
  POST({
    url: 'http://13.234.181.212:8000/third-party-apis/get_crawled_page_data/table',
    data,
  })

export const BacklinkSEOTable = (data: TargetObject[]) =>
  POST({
    url: 'http://13.234.181.212:8000/third-party-apis/backlink-detailed/table',
    data,
  })

export const CompetitorsSEOTable = (data: { domain: string }) =>
  POST({
    url: 'http://13.234.181.212:8000/third-party-apis/get_compititors_website',
    data,
  })
